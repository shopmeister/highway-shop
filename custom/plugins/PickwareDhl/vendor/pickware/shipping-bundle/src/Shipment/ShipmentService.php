...
class ShipmentService
{
    ...
    private function createShipmentBlueprints(
        ...
        foreach ($orders as $order) {
            ...
            $parcel = $this->parcelHydrator->hydrateParcelFromOrder(
                orderId: $order->getId(),
                productsInParcel: $productsInParcelByOrderId[$order->getId()],
                context: $context,
            );
            ...
            $correctedAddress = $this->addressCorrectingService->correctAddress($shipmentBlueprint->getReceiverAddress());
            $shipmentBlueprint->setReceiverAddress($correctedAddress);
    ...
    private function checkPaymentMethodsAreCashOnDeliveryEnabled(EntityCollection $orderTransactions, array $codEnabledPaymentMethodIds): bool
    {
        if ($orderTransactions->count() !== 1) {
            return false;
        }

        return in_array($orderTransactions->getAt(0)->getPaymentMethodId(), $codEnabledPaymentMethodIds);
    }
