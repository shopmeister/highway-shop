...
class Client extends \Amazon\Pay\API\Client
{
     ...
     * @throws \Exception
     */
    public function closeChargePermission($chargePermissionId, $chargePermission, $headers = null)
    {
        if ($chargePermission instanceof ChargePermission) {
            $chargePermission = $chargePermission->toArray();
        }
        $result   = parent::closeChargePermission($chargePermissionId, $chargePermission, $headers);
        $response = json_decode($result['response'], true);
        if ($result['status'] < 200 || $result['status'] > 299) {
            throw new AmazonPayException('closeChargePermission failed: ' . $response['message'] . ' - ' . $response['reasonCode']);
        }
