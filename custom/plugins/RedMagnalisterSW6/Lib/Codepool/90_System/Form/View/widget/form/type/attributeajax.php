...
if (!class_exists('ML', false))
    throw new Exception();
/*
 * example 1: $aField['ajax']['field'] is initial setted (ajax will not executed by initial load)
 *  $aField['type']='ajax';
 *  $aField['ajax']=array(
 ...
 *  );
 * 
 * example 2: $aField['ajax']['field'] is not initial setted (ajax will executed by initial load)
 *  $aField['type']='ajax';
 *  $aField['ajax']=array(
 *      'selector'=>'#'.$this->getField('masterFieldName','id'),//input field css-selector which trigger ajax
 *      'trigger'=>'change',// trigger-type (optional, default=change)
 ...
 */
//    try{
$aMyField = $this->getSubField($aField, 'ajax');
//    }catch(Exception $oEx){
//        $aMyField=array(
//            
//        );
...
if (MLHttp::gi()->isAjax() && (!isset($aMyField['cascading']) || $aMyField['cascading'] != true)) {
    ...
    $sDataAjaxController = htmlentities(json_encode(
        array(
            ...
            'selector' => $aField['ajax']['selector']
        )
    ));
    ?>
    <div class="magnalisterAttributeAjaxForm" data-ajax-trigger="<?php echo isset($aField['ajax']['field']) ? 'false' : 'true' ?>"
