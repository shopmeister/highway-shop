...
?>
    ...
    <table class="datagrid">
        ...
        <thead>
        <tr>
            <th colspan="<?php echo count($aActions) ?>">Actions</th>
        </tr>
        </thead>
        <tbody>
        <?php
        ...
        <tr class=""><?php
            ...
            as $sDo) {
            $cronName = str_ireplace(MLShop::gi()->getShopSystemName(), '', $sDo);
            $cronName = preg_replace('/([A-Z])/', ' $1', $cronName)
            ?>
            <th><?php
                try {

                    ?>
                    <a class="ml-js-noBlockUi" style="float:left;" target="_blank"
                       href="<?php echo $this->getUrl(array('do' => $sDo, 'offset' => 0)); ?>"><?php echo $cronName; ?>
                    (Backend/Admin)</a><?php
                    ?>
                <a class="ml-js-noBlockUi" style="float:right;" target="_blank"
                   href="<?php echo $this->getFrontendDoUrl(array('do' => $sDo, 'auth' => md5(MLShop::gi()->getShopId() . trim(MLDatabase::factory('config')->set('mpid', 0)->set('mkey', 'general.passphrase')->get('value'))), 'offset' => 0, 'blDebug' => 'true')); ?>">
                    <?php echo $cronName; ?> (Frontend)</a><?php
                } catch (Exception $oEx) {
                    $aExceptions[] = $oEx;
                    echo $sDo;
                }
