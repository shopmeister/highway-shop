...
class ML_Sync_Controller_Frontend_Do_CacheAPICalls extends ML_Core_Controller_Abstract {
    ...
    public function execute() {
        ...
        try {
            $this->out(
                '#######################################'."\n##\n".
                '## '.(
                defined('ML_LABEL_API_CALLS_CACHE_LOG')
                    ? ML_LABEL_API_CALLS_CACHE_LOG
            ...
            foreach (MLHelper::gi('Marketplace')->magnaGetInvolvedMarketplaces() as $sMarketPlace) {
                foreach (MLHelper::gi('Marketplace')->magnaGetInvolvedMPIDs($sMarketPlace) as $iMarketPlace) {
                    if ($iRequestMp === null || $iRequestMp == $iMarketPlace) {
                        ...
                        try {
                            $oService = MLService::getCacheAPICallsInstance();
