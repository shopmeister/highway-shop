        ...
        <tr>
            <td class="nowrap" style="width: 5px;">
                <input type="checkbox" id="selectAll"/><label
                    for="selectAll"><?php echo $this->__('ML_LABEL_CHOICE') ?></label>
            </td>
        ...
        if (empty($this->aData)) {
            ...
            foreach ($this->aData as $item) {
                ...
                <tr class="<?php echo(($oddEven = !$oddEven) ? 'odd' : 'even') ?>">
                    <td>
                        <input type="checkbox" name="<?php echo MLHttp::gi()->parseFormFieldName('SKUs[]') ?>"
                               value="<?php echo $item['ArticleSKU'] ?>">
                        <input type="hidden"
                               name="<?php echo MLHttp::gi()->parseFormFieldName("details[{$item['ArticleSKU']}]") ?>"
                               value="<?php echo $sDetails ?>">
                    </td>
                    <?php
