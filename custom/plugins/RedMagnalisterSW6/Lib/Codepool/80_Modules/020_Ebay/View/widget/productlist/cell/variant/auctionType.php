...
if ($this instanceof ML_Productlist_Controller_Widget_ProductList_Abstract) {
    try {
        ...
        $mListingType = $this->getPreparedInfo($oProduct, 'listingtype');
        if (is_array($mListingType) && !empty($mListingType)) {
            $sValue = current($mListingType);
            echo isset($aI18n[$sValue]) ? $aI18n[$sValue] : '';
        }
    } catch (Exception $oEx) {
        echo $oEx->getMessage();
    }
