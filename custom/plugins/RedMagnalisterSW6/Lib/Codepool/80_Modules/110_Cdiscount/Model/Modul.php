...
class ML_Cdiscount_Model_Modul extends ML_Modul_Model_Modul_Abstract {
    public function __construct()
    {
        ...
        MagnaConnector::gi()->setAddRequestsProps(array(
            'SUBSYSTEM' => 'Octopia',
            'SALESCHANNEL'=> $this->getMarketPlaceName(),
    ...
    /**
     * Retrieve SellerId from Octopia if not set.
     *
     * @param bool $blResetCache
     * @return bool
     */
    public function isAuthed($blResetCache = false)
    {
        if (null === $this->getConfig('sellerid')) {
            try {
                $response = MagnaConnector::gi()->submitRequest(['ACTION' => 'IsAuthed']);

                if (!empty($response['DATA']['SellerId'])) {
                    $this->setConfig('sellerid', $response['DATA']['SellerId']);
                } else {
                    $this->setConfig('sellerid', '');
                }
        ...
        return parent::isAuthed($blResetCache);
    }
}
