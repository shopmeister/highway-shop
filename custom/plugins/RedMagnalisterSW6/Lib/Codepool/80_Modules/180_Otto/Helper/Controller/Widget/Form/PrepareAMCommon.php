...
class ML_Otto_Helper_Controller_Widget_Form_PrepareAMCommon extends ML_Form_Helper_Controller_Widget_Form_PrepareAMCommon {
    ...
    public function getCategoryDetails($sCategoryId) {
        if ($sCategoryId !== $this->categoryIndependentAttributes) {
            $result = parent::getCategoryDetails($sCategoryId);
        } else {
            $result = array();
        }
        return $result;
