    ...
    <table class="datagrid ml-plist-old-fix">
        ...
        <tr>
            <td class="nowrap" style="width: 5px;">
                <input type="checkbox" id="selectAll" disabled="disabled"/><label for="selectAll"><?php echo $this->__('ML_LABEL_CHOICE') ?></label>
            </td>
            ...
            if (empty($this->aData)) {
                ...
                foreach ($this->aData as $item) {
                    ...
                    <tr <?php echo $addStyle ?>>
                        <td>
                            <input type="checkbox" name="<?php echo MLHttp::gi()->parseFormFieldName('SKUs[]') ?>" value="<?php echo $item[$this->sDeleteKey] ?>" disabled="disabled">
