...
/** Cross Borders Limitations */
MLI18n::gi()->ML_METRO_CROSS_BORDERS_STOCK_LIMITATION_WARNING = '<strong>Editing stock settings is not possible:</strong> You must configure these in the <a href="{#TAB_LINK#}">tab {#TAB_LABEL#}</a>. The configurations made there are universally applied across all connected METRO marketplaces sharing the same account and country of origin.<br><br>
Ensuring the flawless synchronization of inventory levels with the shop across the METRO marketplaces hinges on this centralized setting.<br><br>
Should you prefer to manage inventory levels for each METRO marketplace individually, please turn off the stock synchronization feature in the <a href="{#TAB_LINK#}">{#TAB_LABEL#} tab</a>, found under "Price and Stock" -> "Stock Sync to Marketplace". This action will unlock the fields that are currently non-editable because they are grayed out.';
MLI18n::gi()->ML_METRO_CROSS_BORDERS_STOCK_LIMITATION_TOOLTIP = 'This field cannot be edited here. For more information, see the note at the top of the screen.';
MLI18n::gi()->ML_METRO_CROSS_BORDERS_LIMITATION_TITLE = 'Metro Cross Borders Limitation';
MLI18n::gi()->ML_METRO_CROSS_BORDERS_ACTIVATE_STOCK_SYNC_INFO = 'Changing the shipping country will reactivate the inventory settings. Following this change, please ensure that the values for "Inventory Item Stock" and "Limitation for Inventory Item Stock" in the "Price and Stock" section, and "Stock Sync to Marketplace" in the "Inventory Synchronization" section, are accurate.';
MLI18n::gi()->ML_METRO_CROSS_BORDERS_DEACTIVATE_STOCK_SYNC_INFO = 'There is already an active configuration for the selected shipping country under <a href="{#TAB_LINK#}">Tab {#TAB_LABEL#}</a>. By agreeing, the inventory settings for this connection will be disabled since they are managed through <a href="{#TAB_LINK#}">Tab {#TAB_LABEL#}</a>.';
