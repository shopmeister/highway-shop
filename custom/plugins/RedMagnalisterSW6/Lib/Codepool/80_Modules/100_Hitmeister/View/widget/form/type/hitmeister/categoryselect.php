    ...
    ?>
        <div class="ml-modal" id="modal-<?php echo $aSubField['id']; ?>" title="Kategorie-Auswahl">
            ...
            <span class="ml-js-ui-dialog-titlebar-additional">
                <span class="last-sync-category"><?php echo MLI18n::gi()->get('ML_GENERIC_CATEGORIES_LAST_SYNC'); ?>: <br>
                    <span class='last-category-import-date'></span>
                </span>
                <a title="<?php echo MLI18n::gi()->get('ML_GENERIC_IMPORT_CATEGORIES', array('marketplace' => MLModule::gi()->getMarketPlaceName(false))) ?>" id="importCategories" class="ui-icon ui-corner-all ui-state-focus global-ajax ui-icon-arrowrefresh-wrap ml-js-noBlockUi" href="<?php echo MLHttp::gi()->getUrl(array('mpid' => MLModule::gi()->getMarketPlaceId(), 'do' => 'ImportCategories')); ?>">
                    <span class="ui-icon ui-icon-arrowrefresh-1-n">reload</span>
                </a>
            </span>
            <?php $this->includeView('do_categories_childcategories', array('sParentId' => 0, 'sType' => $sType, 'sSearchId' => isset($aSubField['value']) ? $aSubField['value'] : '', 'name' => '', 'value' => '')); ?>
        </div>
    <?php
...
<?php
    try {
        ...
        $sql = "SELECT ImportOrUpdateTime FROM $tableName WHERE Site = '".MLDatabase::getDbInstance()->escape($sSite)."' ORDER BY ImportOrUpdateTime DESC LIMIT 1";
        ...
        if ($result) {
            ...
            $lastImportTime = $date->format(MLI18n::gi()->get('ML_DATETIME_FORMAT'));
        }
                            ...
                            buttons: [
                                {
                                    ...
                                    "class": 'mlbtnok',
                                    "click": function () {
                                        // Get selected category from the tree (radio button)
                                        var selectedRadio = jqml(this).find('input[type=radio]:checked');

                                        if (selectedRadio.length === 0 || !selectedRadio.val()) {
                                            alert('<?php echo addslashes(MLI18n::gi()->get('ML_LABEL_CHOOSE')); ?>');
                                            return;
                                        }

                                        var categoryId = selectedRadio.val();
                                        var categoryText = selectedRadio.attr('title') || categoryId;

                                        // Update the select element
                                        if (eSelect.find("option[value='" + categoryId + "']").length == 0) {
                                            eSelect.append('<option value="' + categoryId + '">' + categoryText + '</option>');
                                        }
                                        eSelect.val(categoryId).change();
                                        jqml(this).dialog("close");
                                    }
                                }
                            ]
                    ...
                    jqml('#importCategories').click(function(){
                        ...
                        currentA.magnalisterRecursiveAjax({
                            ...
                            oI18n:{
                                ...
                                sSuccess    : '<?php echo $this->__s('ML_GENERIC_IMPORT_CATEGORIES_SUCCESS',array('\'')) ?>'
                            },
                            onFinalize: function(){
