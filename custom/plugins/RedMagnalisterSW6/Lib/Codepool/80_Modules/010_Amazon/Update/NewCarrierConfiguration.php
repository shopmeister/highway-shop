 ...
 */
class ML_Amazon_Update_NewCarrierConfiguration extends ML_Core_Update_Abstract {

    public function execute() {
        try {

            if (MLDatabase::getDbInstance()->tableExists('magnalister_config')) {
                $aListOfMarketplaces = MLHelper::gi('marketplace')->magnaGetIDsByMarketplace('amazon');
                if (is_array($aListOfMarketplaces)) {
                    foreach ($aListOfMarketplaces as $sMarketplaceId) {
                        ...
                        if ($mOldValue !== null) {
                            MLDatabase::factory('config')->set('mpID', $sMarketplaceId)->set('mkey', 'orderstatus.carrier.select')->set('value', $mOldValue)->save();
                            $oModel->delete();
                        }
                    }
                }
