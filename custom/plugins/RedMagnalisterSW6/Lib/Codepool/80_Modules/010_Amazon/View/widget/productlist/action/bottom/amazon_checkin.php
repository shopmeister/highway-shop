...
<?php if ($this instanceof ML_Productlist_Controller_Widget_ProductList_Abstract) { ?>
    ...
    <div class="ml-container-action">
        <div class="ml-container-inner ml-container-sm" style="background-color: #00d768"></div>
        <div class="ml-container-inner ml-container-md">
            <form class="right" action="<?php echo $this->getCurrentUrl() ?>" method="post"
                ...
                <input type="submit" value="<?php echo $this->__('ML_BUTTON_LABEL_CHECKIN_ADD') ?>" class="js-marketplace-upload mlbtn-red action ml-js-noBlockUi" />
            </form>
            <form class="right" action="<?php echo $this->getCurrentUrl() ?>" method="post"
                  title="<?php echo MLI18n::gi()->ML_STATUS_FILTER_SYNC_ITEM ?>">
                <?php foreach (MLHttp::gi()->getNeededFormFields() as $sName => $sValue) { ?>
                ...
                <input type="submit" value="<?php echo $this->__('ML_BUTTON_LABEL_CHECKIN_PURGE') ?>" class="js-marketplace-upload mlbtn-gray ml-js-noBlockUi" />
            </form>
        </div>
    </div>
    <div id="infoDiag" class="dialog2" title="<?php echo $this->__('ML_LABEL_INFORMATION') ?>"></div>
    <?php
    $this->includeView('widget_upload_ajax', array(
