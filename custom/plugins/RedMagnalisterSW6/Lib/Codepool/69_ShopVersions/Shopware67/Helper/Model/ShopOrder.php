...
class ML_Shopware67_Helper_Model_ShopOrder extends ML_Shopware6_Helper_Model_ShopOrder {
    ...
    protected function handleOrderEvents($aData) {
        ...
        try {
            $context = $this->getSalesChannelContextWithOrderParameter($aData);
            $OrderFixedQuantity = $this->oOrder->getShopOrderObject();
            $event = new CheckoutOrderPlacedEvent($context, $OrderFixedQuantity);
    ...
    public function getLanguageInfo()
    {
        ...
        $oDefaultLanguage = MLShopware6Alias::getRepository('language')
            ->search((new Criteria([Defaults::LANGUAGE_SYSTEM]))->addAssociation('locale'), $context)->first();
        if (isset($oDefaultLanguage)) {
            $languageInfo ??= new LanguageInfo($oDefaultLanguage->getName(), $oDefaultLanguage->getLocale()->getCode());
        }
        return $languageInfo;
    }
    /**
    ...
    protected function getContextWithOrderParameter($aData): Context {
        ...
        $salesChannelContext = $this->getSalesChannelContextWithOrderParameter($aData);
     ...
     * @throws Exception
     */
    protected function getSalesChannelContextWithOrderParameter($aData):SalesChannelContext  {
        $currencyCode = $aData['Order']['Currency'];
        $aConfig = MLModule::gi()->getConfig();
        $iLangId = Uuid::isValid($aConfig['lang']) ? $aConfig['lang'] : null;
        $currency = MLShopware6Alias::getRepository('currency')
        ...
        // Create SalesChannelContext with all required parameters
        ...
        // Instead of returning a regular Context, return the SalesChannelContext
        return $salesChannelContext;
    }
