...
class PdfRenderer
{
    ...
    {
        $dompdf = new Dompdf();
        $extensions = $document->getExtensions();
        ...
        if ($this->dispatcher) {
            $this->dispatcher->dispatch(new BeforeRenderDomPdfEvent($dompdf, $document));
        }
        ...
        $this->logger->logExecutionDuration(function () use ($dompdf) {
            $dompdf->render();
        }, "PDF rendering duration: %s ms");
    ...
    public function renderPdfByHtml($html, $dompdfOptions = []): string
    {
        $dompdf = new Dompdf();
        $options = new Options(array_merge($this->dompdfOptions, $dompdfOptions));
        $dompdf->setOptions($options);
        $dompdf->setPaper('A4');
        $dompdf->loadHtml($html);

        if ($this->dispatcher) {
            $rendered = new RenderedDocument($html);
            $this->dispatcher->dispatch(new BeforeRenderDomPdfEvent($dompdf, $rendered));
        }
